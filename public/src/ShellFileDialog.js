(function() {
  ...
  Shell.create.fileOpenDialog = function(shell,app,opt){
    return new FileOpenDialog(shell,app,opt);
  }

  Shell.create.fileSaveDialog = function(shell,app,opt){
    return new FileSaveDialog(shell,app,opt);
  }
  ...
  class FileDialog {
    constructor(shell,{title,root,path,ok,types,cancel}) {
      ...
      this.treeView = this.shell.create.treeView({
        container: this.$directories,
        root:this.root,
        cwd:this.path,
        on: {
          cd: (cwd) => {
